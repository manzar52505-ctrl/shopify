...
export const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  return (
    <div 
      className={`group relative rounded-3xl overflow-hidden hover:scale-[1.02] hover:-translate-y-1 transition-all duration-500 cubic-bezier(0.25, 0.8, 0.25, 1) cursor-pointer border 
        ${isSwap 
          ? 'bg-gradient-to-b from-white to-orange-50 dark:from-slate-900 dark:to-orange-900/10 border-orange-100 dark:border-orange-500/30 hover:border-orange-300 dark:hover:border-orange-500/50 hover:shadow-2xl hover:shadow-orange-500/20' 
          : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-indigo-700/50 hover:shadow-2xl hover:shadow-indigo-500/20'
        }`}
    ...
    >
      ...
      <div className="relative w-full aspect-square overflow-hidden bg-slate-100 dark:bg-slate-800">
        ...
        <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${isSwap ? 'bg-gradient-to-t from-orange-900/40 via-transparent to-transparent' : 'bg-gradient-to-t from-slate-900/40 via-transparent to-transparent'}`}></div>

        {/* Top Badges */}
        <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
          {isSwap ? (
            <div className="bg-orange-500/90 backdrop-blur-md px-3 py-1.5 rounded-full text-xs font-bold text-white flex items-center gap-1.5 shadow-lg border border-white/20">
        ...
        {isSwap && onRequestSwap && (
          <button
            ...
            className="absolute bottom-3 left-3 px-4 h-10 bg-orange-500 text-white rounded-full flex items-center gap-2 shadow-lg translate-y-16 group-hover:translate-y-0 transition-transform duration-300 delay-75 hover:bg-orange-600 z-10 border border-white/20"
          >
            <RefreshCw size={16} />
      ...
      <div className="p-5 relative">
        {isSwap && (
          <RefreshCw className="absolute right-3 bottom-3 text-orange-500/5 rotate-12 pointer-events-none transition-transform group-hover:rotate-45 duration-700" size={80} />
        )}

        <div className="mb-2">
           <h3 className={`font-bold text-base leading-snug line-clamp-1 transition-colors ${isSwap ? 'text-slate-900 dark:text-white group-hover:text-orange-600 dark:group-hover:text-orange-400' : 'text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400'}`}>
            {product.name}
          </h3>
          {product.addedBy?.email && (
            <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 truncate">
              Listed by {product.addedBy.name.split(' ')[0]}
            </p>
          )}
        </div>
        
        <div className="flex items-center justify-between relative z-10">
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">{product.category}</p>
          
          {isSwap ? (
             <span className="text-xs font-bold text-orange-600 dark:text-orange-400 uppercase tracking-wide bg-orange-100 dark:bg-orange-900/30 px-2.5 py-1 rounded-md border border-orange-200 dark:border-orange-800/30">
               Swap
             </span>
          ) : (
